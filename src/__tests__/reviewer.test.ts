...
describe('runHolisticReview', () => {
  ...
  it('correctly builds prompt containing spec content, CLAUDE.md, and task summary', async () => {
    ...
    expect(prompt).toContain('Spec completeness');
    expect(prompt).toContain('Convention adherence');
  });
  ...
  it('saves result to .cloudy/review-latest.json', async () => {
    await runHolisticReview(cwd, makePlan(), 'sonnet');

    expect(vi.mocked(writeJson)).toHaveBeenCalledWith(
      path.join(cwd, '.cloudy', 'review-latest.json'),
