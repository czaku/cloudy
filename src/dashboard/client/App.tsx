...
let _lineId = 0;
function nextLineId() { return `l${++_lineId}`; }

function toolHintFrom(input: unknown): string {
  if (!input || typeof input !== 'object') return '';
  const i = input as Record<string, unknown>;
  return String(i['file_path'] ?? i['command'] ?? i['path'] ?? i['url'] ?? i['pattern'] ?? i['query'] ?? '');
}
...
function reducer(state: AppState, action: Action): AppState {
  switch (action.type) {
    ...
    case 'task_started': {
      ...
      // Show the task prompt as a "user" message so the conversation makes sense
      const task = findTask(plan, action.taskId);
      const promptLines: OutputLine[] = task?.description
        ? [{ id: nextLineId(), type: 'prompt', taskId: action.taskId, content: `**${action.title}**\n${task.description}`, toolName: action.model }]
        : [];
      const eventLine = makeEventLine(
        action.taskId,
        `▶ Attempt ${action.attempt}/${action.maxAttempts} · ${action.contextFileCount} context files${action.engine ? ` · ${action.engine}` : ''}`,
        'event',
      );
      return { ...state, plan, outputLines: appendLines(state.outputLines, [...promptLines, eventLine]) };
    }
    ...
    case 'task_tool_call': {
      const hint = toolHintFrom(action.toolInput);
      const line: OutputLine = {
        ...
        toolHint: hint,
        content: JSON.stringify(action.toolInput, null, 2),
      };
  ...
  return (
    <div className="app-root">
      ...
      <OutputLog
        ...
        onClear={clearOutput}
        failedTaskId={state.lastFailedTaskId}
        costSummary={state.costSummary}
        tasks={state.plan?.tasks ?? []}
