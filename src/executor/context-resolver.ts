        ...
        if (!seen.has(filePath) && stat.size <= 100_000) {
          ...
          if (budgetTokens > 0 && (totalChars + content.length) / 4 > budgetTokens) {
            if (budgetMode === 'enforce') {
              throw new Error(`Context budget exceeded (${budgetTokens} tokens) while loading ${relPath} — set contextBudgetMode to 'warn' or increase contextBudgetTokens`);
            }
    ...
    for (const relFile of allFiles) {
      if (matchGlob(relFile, pattern)) {
        ...
        try {
          ...
          if (budgetTokens > 0 && (totalChars + content.length) / 4 > budgetTokens) {
            if (budgetMode === 'enforce') {
              throw new Error(`Context budget exceeded (${budgetTokens} tokens) while loading ${relFile} — set contextBudgetMode to 'warn' or increase contextBudgetTokens`);
            }
